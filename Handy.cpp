...
{	
   ...
   if (now != -1)
   {
      strftime(the_date, 18, "%I_%M_%d_%m_%Y", gmtime(&now));
   }
...
//This returns the filename from a path, e.g. /seq/test.bam returns test.bam
//Doesn't work well if the input is hardcoded from code
string Handy::getFileNameFromPath(string longName){
	
	cout<<"Input: "<<longName<<endl;
	std::size_t found;
	string shortName;
	shortName=longName;
	...
	if (found==std::string::npos) // try a windows separator if not unix
	{
		found=longName.find_last_of("\/");
	}
	
	if (found!=std::string::npos && longName.length()>found) 
	{
		shortName=longName.substr(found+1);
	}

	cout<<"output: "<<shortName<<endl;
	
	return shortName;
	
}

//This returns the filename from a path stripped of its type, e.g. /seq/test.bam returns /seq/test 
//but  you'd probably want to run test.bam and get test
string Handy::getFileNameStubFromFileName(string longName){
	
	cout<<"Input Second: "<<longName<<endl;
	...
	if (found!=std::string::npos && longName.length()>found) 
	{
		shortName=longName.substr(0,found);
	}
	
	cout<<"output Second: "<<shortName<<endl;
